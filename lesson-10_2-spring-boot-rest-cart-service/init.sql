...
ALTER TABLE carts_sequence
    OWNER TO postgres;

create table cart
(
    cart_id bigint not null default nextval('carts_sequence') primary key
);
...
create table item
(
    item_id    bigint         not null default nextval('items_sequence') primary key,
    quantity   integer        not null,
    price      numeric(19, 2) not null,
    cart_id    bigint         not null
        constraint fk_item_cart_cart_id
            references cart,
    product_id bigint         not null
        constraint fk_item_product_product_id
            references product
);
